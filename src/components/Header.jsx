    ...
    <nav className={`${darkMode ? 'dark bg-black' : 'light bg-[#f3f3f3]'} flex justify-between items-center gap-4 lg:px-20 px-4 py-3 sticky top-0 z-30`}>
      ...
      <ul className='lg:flex justify-center items-center gap-8 hidden'>
        {navItems.map(({ link, path }) => (
          <li key={path}>
            <Link
              className={`text-[15px] uppercase font-semibold cursor-pointer px-3 py-2 rounded-lg hover:bg-red-600 hover:text-white ${darkMode ? 'text-white' : 'text-black'}`}
              to={path}
              spy={true}
              offset={-100}
              smooth={true}
      ...
      <div className='flex justify-center items-center lg:hidden' onClick={toggleMenu}>
        <div>
          {isMenuOpen ? <FaXmark className='text-black light:text-white text-xl cursor-pointer'/> : <FaBars className='text-black light:text-white text-2xl cursor-pointer'/>}
        </div>
      </div>

      <div className={`${isMenuOpen ? 'flex' : 'hidden'} w-full h-fit bg-slate-800 p-4 abosulate top-[80px] left-0`} onClick={closeMenu}>
      ...
      <div className='flex justify-center items-center lg:gap-8 gap-2'>
        <div className='flex justify-center items-center lg:gap-3 gap-1'>
          ...
          <h1 className='lg:text-xl text-sm text-black dark:text-red-600 font-semibold'>9912344618 </h1>
